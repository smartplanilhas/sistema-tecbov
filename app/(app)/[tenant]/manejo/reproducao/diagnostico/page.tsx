...
import { DiagnosticoForm } from '../registro-view'
...
export default async function DiagnosticoPage({ params }: { params: Promise<{ tenant: string }> }) {
  ...
  const ultimaCoberturaMap    = new Map<string, string>()
  for (const ev of (eventosRes.data ?? [])) {
    if (!ultimoEventoPorAnimal.has(ev.animal_id)) ultimoEventoPorAnimal.set(ev.animal_id, ev)
    if ((ev.tipo === 'inseminacao' || ev.tipo === 'monta_natural') && !ultimaCoberturaMap.has(ev.animal_id)) {
      ultimaCoberturaMap.set(ev.animal_id, ev.data)
    }
  }
  ...
  const femeas = (femeasRes.data ?? []).map((a: any) => ({
    ...
    ultima_cobertura:    ultimaCoberturaMap.get(a.id) ?? null,
  }))
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-muted-foreground text-sm mt-1">
          Registre diagnósticos de gestação em uma ou mais fêmeas.
        </p>
      </div>
      <div className="rounded-xl border bg-card p-6">
        <DiagnosticoForm tenantSlug={tenantSlug} femeas={femeas} lotes={lotes} />
      </div>
    </div>
  )
}
