...
import { DesmameForm } from '../registro-view'
...
function derivarStatusSimples(ev: { tipo: string; data: string; dados: any } | null): string {
  ...
  if (ev.tipo === 'aborto' || ev.tipo === 'desmame') return 'Vazia'
  return 'Vazia'
}

export default async function DesmamePage({ params }: { params: Promise<{ tenant: string }> }) {
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-muted-foreground text-sm mt-1">
          A data de desmama será atualizada automaticamente no registro do bezerro.
        </p>
      </div>
      <div className="rounded-xl border bg-card p-6">
        <DesmameForm tenantSlug={tenantSlug} femeas={femeas} lotes={lotes} />
      </div>
    </div>
  )
}
