...
export function TransactionDialog({
  ...
  const coaType = type === 'INCOME' ? 'REVENUE' : type === 'EXPENSE' ? 'EXPENSE' : null

  const analyticalItems = categories.filter(
    (c) => !c.is_group && (coaType === null || c.type === coaType)
  )
        ...
        <form onSubmit={handleSubmit} className="space-y-4 mt-2">
          <div className="grid grid-cols-2 gap-4">
            <div className="space-y-1.5">
              <Label>Tipo</Label>
              <Select value={type} onValueChange={(v) => { setType(v); setCategoryId('') }} required>
                ...
                <SelectContent>
                  ...
                  {analyticalItems.length === 0 && (
                    <p className="px-2 py-4 text-center text-xs text-muted-foreground">
                      {coaType ? 'Nenhuma categoria disponível' : 'Selecione o tipo primeiro'}
                    </p>
                  )}
                </SelectContent>
              </Select>
          ...
          <DialogFooter>
            ...
            <Button type="submit" disabled={loading || !type || !accountId}>
